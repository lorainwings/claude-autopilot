...
| figma-codegen | — | 纯 Skill 无运行时 | 仅 `make fc-lint` shellcheck |
| slim-task | — | 纯 Skill 无运行时 | 仅 `make st-lint` shellcheck |

> 纯 Skill 插件（daily-report / figma-codegen / slim-task）只对 `tools/build-dist.sh` 做 shellcheck，**不要求**运行时单测；SOP 与产物质量由各插件 SKILL.md 自带的检查点和 Phase 5 审计（slim-task）等机制保证。
