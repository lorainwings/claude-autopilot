...
- **证据**：`engineering-sync-gate.sh:62-89` 显式调用 `detect-anchor-drift.sh`、读取 `.cache/spec-autopilot/anchor-drift-candidates.json`、合并进 `.cache/spec-autopilot/engineering-sync-report.json`
- **结论**：无需额外集成工作
...
2. **SKILL.md 示例 glob 明确穿透到 `.sh`**（Finding 1.4）：避免用户 copy-paste 到目录 glob
