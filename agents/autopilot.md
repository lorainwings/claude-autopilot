...
1. 解析 `$ARGUMENTS`：文件路径 → 读取；文本 → 直接使用；空 → AskUserQuestion
2. Task 调用 config.phases.requirements.agent（默认 business-analyst）梳理需求
3. 至少一轮 AskUserQuestion Q&A 确认功能范围
4. 整理结构化提示词（背景、目标、用例、约束、验收标准）
5. AskUserQuestion 确认需求理解
