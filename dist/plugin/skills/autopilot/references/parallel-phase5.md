...
for each agent in sorted(agents, key=task_number):
  ...
  Bash('bash ${PLUGIN_ROOT}/scripts/emit-task-progress.sh "task-{N}-{slug}" running {N} {total} {mode}')
  ...
  Bash('bash ${PLUGIN_ROOT}/scripts/emit-task-progress.sh "task-{N}-{slug}" passed {N} {total} {mode}')
