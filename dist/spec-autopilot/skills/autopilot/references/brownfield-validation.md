...
> 由 autopilot-gate SKILL.md 引用。通过 `config.brownfield_validation.enabled` 控制（默认 `true`（greenfield 项目由 Phase 0 自动关闭））。
...
brownfield_validation:
  enabled: true               # 默认开启，greenfield 项目由 Phase 0 自动关闭
