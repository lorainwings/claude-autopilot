...
> **v6.x 与 depth gating 的关系**：自 v6.x 起，`config.phases.requirements.research.depth` 是联网调研的**主开关**——`depth=standard` 时 `websearch_quota=0`，本规则引擎不会触发任何 WebSearch；只有当 `depth=deep`（或被 maturity=ambiguous / complexity=large 自动升级）时，下述 `search_policy` 才作为**辅助决策层**参与 query 数量与跳过判定。`search_policy` 字段在 standard 路径下属于遗留兼容字段，不影响实际执行。
...
- Read: openspec/changes/<name>/context/research-findings.md
  （v6.x 起联网调研结果合并写入该文件的 `## Web Research Findings` 章节，不再单独维护 `web-research-findings.md`）
