...
4. **汇合调研结果**（v3.3.0 上下文保护增强）→ 子 Agent 自行 Write 产出文件 + 返回结构化 JSON 信封（含摘要和决策点），主线程**不读取全文**：
...
10. 可配置用户确认点（`config.gates.user_confirmation.after_phase_1`，**v6.0 默认 false**——需求评审完成后默认自动推进）
...
11. **发射 Phase 1 结束事件（v5.2 Event Bus 补全, v6.0 H-1 补全 requirement_packet_hash）**:
...
当 `mode === "full"` 且 `tdd_mode !== true` 且 `phase4_test_files` 非空时，主线程对每个 task 执行 L2 测试驱动验证（详见 `references/phase5-implementation.md` 非 TDD L2 验证章节）：
...
**并行模式**：主线程无法对域 Agent 内部逐 task L2 验证。改为**合并后全量验证**：
