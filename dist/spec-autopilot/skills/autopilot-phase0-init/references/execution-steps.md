...
- [Step 2: 检查配置文件](#step-2-检查配置文件)
- [Step 3: 解析执行模式](#step-3-解析执行模式)
- [Step 4: 启动 GUI 服务器 + 展示启动 Banner（合并）](#step-4-启动-gui-服务器--展示启动-banner合并)
- [Step 4.5: 初始化事件文件 + 发射 Phase 0 开始事件（Event Bus 补全）](#step-45-初始化事件文件--发射-phase-0-开始事件event-bus-补全)
- [Step 4.6: 注入历史教训（Sprint 升级新增 — 主动学习 Phase 0 入口）](#step-46-注入历史教训sprint-升级新增--主动学习-phase-0-入口)
- [Step 5: 检查必需插件](#step-5-检查必需插件)
- [Step 6: 崩溃恢复](#step-6-崩溃恢复)
- [Step 6.1: 事件文件恢复清理](#step-61-事件文件恢复清理)
- [Step 7: 创建阶段任务（恢复状态增强, 顺序强制）](#step-7-创建阶段任务恢复状态增强-顺序强制)
- [Step 8: 确保锁文件被 gitignore](#step-8-确保锁文件被-gitignore)
- [Step 9: 创建锁文件（确定性脚本）](#step-9-创建锁文件确定性脚本)
- [Step 10: 创建锚定 Commit（确定性更新）](#step-10-创建锚定-commit确定性更新)
- [Step 10.5: 发射 Phase 0 结束事件（Event Bus 补全）](#step-105-发射-phase-0-结束事件event-bus-补全)
