...
- [派发流程](#派发流程)
- [代码质量扫描（内置，始终执行）](#代码质量扫描内置始终执行)
- [结果收集](#结果收集)
- [质量汇总表（Phase 7 展示）](#质量汇总表phase-7-展示)
- [安全审计扫描](#安全审计扫描)
...
运行 `bash ${CLAUDE_PLUGIN_ROOT}/runtime/scripts/check-security-tools-install.sh "$(pwd)"`，获取已安装的安全工具清单。
...
1. **工具检测**:
   运行: bash ${CLAUDE_PLUGIN_ROOT}/runtime/scripts/check-security-tools-install.sh "$(pwd)"
