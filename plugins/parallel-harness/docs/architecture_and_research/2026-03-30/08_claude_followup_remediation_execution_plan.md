...
本文是基于当前这一轮 Claude 修复结果之后的 **二次修复施工单**。

目标不是重新做架构分析，而是把这次 review 中确认的“未彻底修复项”和“只做了表层接线的项”整理成一份可直接交给 Claude 执行的工程方案。

适用方式：

- 直接把本文内容发给 Claude
- 或让 Claude 先阅读本文，再开始在 `plugins/parallel-harness` 下施工

## 2. 本轮必须修复的核心问题

这次二次修复必须完整覆盖以下 8 项，不能只修其中一部分：
...
不允许只新增测试让当前问题“被记录下来”而不修实现。

### 3.2 不能只接接口

不允许继续新增未接线的类或工具函数。凡是新增模块，必须真实进入 runtime 主链。
...
如果某项能力还没有真正落地，文档必须降级表述，不能继续宣传为已完成。
...
你现在需要继续修复 `plugins/parallel-harness`，目标是完成上一轮修复未真正闭环的问题。不要重新做宏观方案，不要只写文档，不要只补测试。你必须直接改代码、补测试、运行验证。
...
4. 调整方法签名，必要时把 `plan` 传入 `determineFinalStatus()`。
...
- 先修状态机真相，避免后续验证失真
- 再修调度和执行主链
- 最后收口 report/control plane/governance

## 7. 每个工作流完成后的固定验证命令

Claude 每完成一个工作流，至少执行：
