...
2. 按优先级选取最高未解决维度，构造**单一问题**：
   - 优先提供 2-4 个具体多选选项 + 1 个"其他（请补充）"
   - 每轮问题的最后一个选项固定为「已足够清晰，无需继续」
   - 通过 `AskUserQuestion` 发送（选项按 `${LANG}`）
...
**检查点**：执行检查点标准协议，选项为「采纳 → Phase 2 / 补充澄清（重入循环）/ 终止」。用户选「补充澄清」时重入 Step 2 循环继续迭代。
...
1. **业务影响分析**：对现有功能的影响等级（无影响 / 低 / 中 / 高）+ 受影响的上下游模块/调用方
2. **回滚方案**：若上线后发现问题，如何安全回退（`git revert` 是否充分 / 是否需要数据迁移回退脚本）
3. **风险与权衡**
