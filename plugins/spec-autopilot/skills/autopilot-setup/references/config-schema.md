...
phases:
  requirements:
    ...
    research:
      ...
      web_search:                # [DEPRECATED] 旧的独立联网 Agent 配置；保留以向后兼容，子键不再生效
        ...
        agent: ""                # [DEPRECATED] 由 research.agent 统一负责，无需独立 agent。
        search_policy:
    ...
    complexity_routing:
      ...
      thresholds:
        small: 2
        medium: 5
    synthesizer:                 # 专职汇总者：把 auto_scan + research + BA 的输出整合为 requirement-packet
...
test_pyramid:
  ...
  traceability_floor: 80     # test_traceability 覆盖率阈值（L2 blocking，默认 80%）
...
quality_scans:               # Phase 6 真实静态分析工具集成（可选）
...
gates:
  user_confirmation:
    after_phase_1: false     # 需求确认后默认自动推进
...
brownfield_validation:
  enabled: true                # 默认开启漂移检测（greenfield 项目 Phase 0 自动关闭）
...
#   semantic_rules:                  # 语义规则（dispatch 注入子 Agent prompt）
#     - rule: "描述规则内容"
...
phases 内必须的 key:
  ...
  - phases.requirements.synthesizer.agent (string, 必填，禁止空值/Explore)
