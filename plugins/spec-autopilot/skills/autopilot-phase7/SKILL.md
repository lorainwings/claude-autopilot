...
**v5.8 block_on_critical 语义澄清**: 在展示归档确认前，如果 `config.phases.code_review.block_on_critical = true`：
1. 检查 `phase-6.5-code-review.json` 中是否存在 `critical` 级别 findings
2. 如有 critical findings 未修复：
   - 展示 critical findings 详情
   - AskUserQuestion 选项变更为（**替代**下方通用归档提问，二者互斥）：
     - "忽略 critical findings，继续归档"
     - "修复 critical findings 后再归档"
     - "暂不归档"
   - 日志输出 `[WARNING] block_on_critical: N critical code review findings unresolved`
   - 用户选择"忽略"后直接进入 Step 4 归档操作，不再弹出通用归档提问
3. 如无 critical findings 或 `block_on_critical = false`：展示下方通用归档确认
...
b. **Git 自动压缩**（当 `config.context_management.squash_on_archive` 为 true，默认 true）：
  ...
  - **v5.8 fixup 完整性检查**: 归档前验证 fixup 提交完整性——对比 checkpoint 数量与 fixup commit 数量：
    ...
    ```
    如果 `FIXUP_COUNT < CHECKPOINT_COUNT`，输出警告（不阻断）：
    `[WARNING] fixup 完整性检查: ${FIXUP_COUNT} fixup commits < ${CHECKPOINT_COUNT} checkpoints. 部分 checkpoint 可能未关联 git fixup 提交。`
  ...
  - 验证 anchor_sha 有效：`git rev-parse $ANCHOR_SHA` → 无效则先尝试重建：
    ...
    3. 退出码非 0 → AskUserQuestion 给用户两个选项：
       - "跳过 autosquash，保留所有 fixup commits 完成归档"
       - "中止归档，手动修复后重试"
