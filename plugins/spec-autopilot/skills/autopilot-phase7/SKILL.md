...
**v5.3 进度写入**: `Bash('bash ${CLAUDE_PLUGIN_ROOT}/runtime/scripts/write-phase-progress.sh 7 summary_dispatched in_progress')`
...
**v5.3 进度写入**: `Bash('bash ${CLAUDE_PLUGIN_ROOT}/runtime/scripts/write-phase-progress.sh 7 summary_complete complete')`
...
b. **Git 自动压缩**（当 `config.context_management.squash_on_archive` 为 true，默认 true）：
  - **v5.3 进度写入**: `Bash('bash ${CLAUDE_PLUGIN_ROOT}/runtime/scripts/write-phase-progress.sh 7 autosquash_started in_progress')`
