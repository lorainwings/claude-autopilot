...
2. **项目上下文扫描**（Auto-Scan）→ 优先读取持久化上下文（`openspec/.autopilot-context/`），7 天内有效则跳过全量扫描仅做增量；否则生成 Steering Documents 并同时持久化
...
autopilot-gate 额外验证（阈值从 config.phases.testing.gate 读取，**按复杂度调整**）：

| 复杂度 | required_test_types | min_test_count_per_type |
|--------|--------------------|-----------------------|
| small | config 值，但允许缺少 ui 类型 | max(2, config 值 / 2) |
| medium | config 值 | config 值 |
| large | config 值 | max(config 值, 5) |

- `test_counts` 每个字段 ≥ 调整后的 min_test_count_per_type
- `artifacts` 包含调整后的 required_test_types 对应文件
- `dry_run_results` 全部为 0（exit code）
- `coverage.coverage_pct` === 100（需求追溯覆盖率，v3.2.0 新增）
- `traceability_matrix` 非空且每个 req_id 有至少一个 test_case

> **v3.1.0 变更**：small 复杂度项目不再强制要求 4 类全部测试，允许缺少 UI 测试。测试数量门禁也按复杂度动态调整。
...
### Phase 5 特殊处理（v3.2.0：Subagent-Driven 并行 + 双阶段 Review）

**参考文档**: `references/phase5-implementation.md`（安全准备、超时机制、降级策略详述）

概要:
1. Git 安全检查点 → `git tag -f autopilot-phase5-start HEAD`
2. 记录启动时间戳 → wall-clock 超时机制（2 小时硬限）
3. **检查并行执行配置** → 读取 `config.phases.implementation.parallel.enabled`
   - **启用** → **读取并执行 `templates/phase5-parallel.md`**（Subagent-Driven 并行模板）
     - **独立域并行**：backend/frontend/node 的 task 分域并行执行（每域一个 Domain Runner subagent）
     - **同域串行**：Domain Runner 内部串行执行各 task，fresh subagent + self-review
     - **双阶段 review**：所有域合并后强制 spec-review + quality-review（`templates/phase5-review-prompts.md`）
     - **Worktree 隔离**：并行时自动启用（无需独立配置 `worktree.enabled`）
     - **域特定 agent**：从 `config.parallel.agent_mapping` 读取各域的专业 agent 类型
     - **降级**：仅 1 个域有 task → 自动切换串行模板
   - **禁用** → 使用下方串行策略
4. 检测 ralph-loop 可用性
5. **ralph-loop 可用** → 构造参数调用 `Skill("ralph-loop:ralph-loop")`，完成后从 test-results.json 构造 JSON 信封
6. **不可用 + fallback 启用** → 手动循环模式（每任务 apply + 测试）
7. **不可用 + fallback 禁用** → AskUserQuestion 让用户选择处理方式

### Phase 5→6 特殊门禁

autopilot-gate 额外验证：`test-results.json` 存在、`zero_skip_check.passed === true`、`tasks.md` 全部 `[x]`

### Phase 6 特殊处理（v3.2.0：并行测试执行）

Phase 6 子 Agent **必须读取并执行 `templates/phase6-parallel.md`**：

1. 将测试套件分为 fast（typecheck）和 heavy（unit/api/e2e/ui）
2. fast 套件在主进程串行执行
3. heavy 套件通过 `Task(run_in_background: true)` 并行派发 — 所有 Task 调用在**同一个 response** 中发出
4. 等待所有 background Task 完成后收集结果
5. 根据 `config.phases.reporting.format` 生成报告：
   - `"allure"` → 合并 Allure 结果 → `npx allure generate` → HTML 报告
   - `"custom"` → 使用 `config.phases.reporting.report_commands`
6. 写入 `test-results.json` 并构造 Phase 6 JSON 信封

> **降级**：如果 Task 并行调用失败，回退为串行执行测试套件。
...
| 复杂度感知 | Phase 4 测试门禁和 Phase 1 流程按 complexity 动态调整（v3.1.0） |
| 文件级锁 | Phase 5 并行执行使用文件级锁防止跨 agent 冲突（v3.1.0） |
| 决策排序 | 决策点按阻断性→不可逆性→影响范围排序，P3 支持快速确认（v3.1.0） |
