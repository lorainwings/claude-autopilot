...
| `phases.implementation` | instruction_files、ralph_loop 配置（enabled/max_iterations/fallback_enabled）、micro_verification |
| `phases.reporting` | instruction_files、report_commands、coverage_target、zero_skip_required |
| `test_suites` | 各测试套件命令和类型 |
| `model_tier` | 各阶段模型配置，缺失时全部 Opus |
| `parallel` | 并行调度配置，缺失或 enabled=false 时全部串行 |
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
[Phase {N}/7] {phase_name} — ⏳ 进行中
前置: ✅ Phase {N-1} | 模型: {model}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
...
Phase 5 额外显示任务进度: `[Task {M}/{total}] task_description`

Phase 完成后更新为: `[Phase {N}/7] {phase_name} — ✅ 完成`
...
当 `config.parallel.enabled` 且 `config.parallel.phase_4.enabled` 为 true 时，主线程在单条消息中同时 dispatch 4 个 Task：
...
主线程收集 4 个返回 → 按 autopilot-checkpoint 并行合并协议合并为统一的 Phase 4 信封 → 写入 checkpoint。

parallel 缺失或 phase_4.enabled = false → 退回单 Agent 串行模式（向后兼容）。
...
当 `config.phases.implementation.micro_verification.enabled` 为 true 时：
- 读取 `config.phases.implementation.micro_verification.commands`
- 在 Phase 5 dispatch prompt 中注入微验证命令列表
- 子 Agent（或 ralph-loop）在每个 task 完成后运行这些命令
- 任何命令失败 → 视为当前 task 失败，触发修复循环

命令列表为空时，降级使用 ralph-loop-config.md 中的硬编码验证命令。
缺失 `micro_verification` 配置节 → 不注入，使用默认行为（向后兼容）。

#### Phase 5 并行任务组（可选）

当 `config.parallel.enabled` 且 `config.parallel.phase_5.enabled` 为 true 时：

```
Step 5.0: 读取 tasks.md，分析任务间的文件依赖关系
  → 对每个任务提取涉及的文件路径
  → 构建依赖图：共享文件的任务必须串行
  → 输出并行组: [[task1, task3], [task2], [task4, task5], ...]

Step 5.1: 对每个并行组，同时 dispatch 多个 Task
  → config.parallel.phase_5.use_worktree = true 时使用 isolation: "worktree"
  → 每个 Agent 在 git worktree 中工作，互不干扰
  → 完成后合并变更到主分支

Step 5.2: 合并结果，运行全量微验证命令
```

安全约束:
- 同一任务组内的任务**必须不共享任何文件**
- 每个并行 Agent 使用 `isolation: "worktree"` 确保文件系统隔离
- 合并时如有冲突，退回串行模式重新执行冲突任务
- parallel 缺失或 phase_5.enabled = false → 退回 ralph-loop 串行模式
...
当 `config.parallel.enabled` 且 `config.parallel.phase_6.enabled` 为 true 时，Phase 6 子 Agent 内部使用后台进程并行执行测试套件：
...
这不需要修改 Hook 或 Gate，只需在 Phase 6 的 dispatch prompt 中指导子 Agent 使用并行执行策略。合并所有结果到 `test-results.json`。

parallel 缺失或 phase_6.enabled = false → 子 Agent 串行执行测试（现有行为）。
...
| 阶段 | 状态 | 模型 | 摘要 |
|------|------|------|------|
| Phase 2 — 创建 OpenSpec | ✅ ok | sonnet | ... |
| Phase 3 — 快进生成制品 | ✅ ok | opus | ... |
| Phase 4 — 测试用例设计 | ✅ ok | opus | ... |
| Phase 5 — 循环实施 | ✅ ok | opus | ... |
| Phase 6 — 测试报告 | ✅ ok | sonnet | ... |
```

模型列从 checkpoint 的 `model_used` 字段读取，缺失时不显示模型列（向后兼容）。
