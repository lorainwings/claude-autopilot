...
0. **写入 Phase 7 Checkpoint（进行中）**：调用 Skill(`spec-autopilot:autopilot-checkpoint`) 写入 `phase-7-summary.json`：
   ...
   ```
1. **派发汇总子 Agent**（v3.3.0 上下文保护增强）→ 前台 Task 读取所有 checkpoint 并生成汇总：
   ```
   Task(subagent_type: "general-purpose", prompt: "
     ...
     执行 bash scripts/collect-metrics.sh 收集执行指标。
     生成以下格式的 JSON 信封返回（禁止返回 checkpoint 原文）：
     {
       \"status\": \"ok\",
       \"phase_summary\": [{\"phase\": N, \"status\": \"ok|warning|blocked\", \"description\": \"...\"}],
       \"test_report\": {\"format\": \"allure|custom\", \"path\": \"...\", \"suites\": [{\"name\": \"...\", \"total\": N, \"passed\": N, \"failed\": N, \"skipped\": N}], \"anomaly_alerts\": \"...\"},
...
2. **收集三路并行结果**（Phase 6 三路并行的汇合点，**仅 full/lite 模式**）：
   ...
   **等待机制**：阻塞等待路径 A/B/C 的后台 Agent 完成或超时（`config.background_agent_timeout_minutes`，默认 30 分钟），Claude Code 会自动发送完成通知。超时自动标记 `"timeout"`，不阻断 Phase 7 流程。
   ...
   a0. **Phase 6 测试执行**（路径 A，后台 Task）：
      - 等待后台 Agent 完成通知
      - 解析 JSON 信封，按统一调度模板 Step 4/5+7/6/8 处理（后台 Checkpoint Agent 写入 + TaskUpdate + 等待确认）
...
2.5. **Allure 本地预览**（v3.4.3，仅当 `config.phases.reporting.format === "allure"` 且 allure-report 目录存在时）：
   - 后台启动本地预览服务器：`Bash('npx allure open allure-report', run_in_background: true)`
   - 向用户展示访问地址：`Allure Report: http://localhost:<port>/`
   - 服务器随 Phase 7 归档完成后自动清理（或用户手动关闭）
3. **必须** AskUserQuestion 询问用户：
   ...
   ```
4. 用户选择"立即归档"：
   a. **归档前清理**（在 git 提交之前完成，确保最终 commit 包含干净状态）：
      - **更新 Phase 7 Checkpoint（完成）**：调用 Skill(`spec-autopilot:autopilot-checkpoint`) 更新 `phase-7-summary.json`：
        ```json
        {"status": "ok", "phase": 7, "description": "Archive complete", "archived_change": "<change_name>", "mode": "<mode>"}
        ```
      - **删除临时文件**：删除 `openspec/changes/<name>/context/phase-results/phase5-start-time.txt`（如果存在）
   b. **Git 自动压缩**（当 `config.context_management.squash_on_archive` 为 true，默认 true）：
      - **提交工作区残留变更**：执行 `git add -A && git diff --cached --quiet || git commit --fixup=$ANCHOR_SHA -m "fixup! autopilot: start <change_name> — final"`，确保所有变更（含 phase-7 checkpoint 和临时文件删除）已纳入 git
      - 读取 `${session_cwd}/openspec/changes/.autopilot-active` 中的 `anchor_sha`
...
7. **清理锁定文件**：删除 `${session_cwd}/openspec/changes/.autopilot-active`（无论用户选择何种归档方式）
8. **清理 git tag**：删除 `autopilot-phase5-start` tag（如果存在）：`git tag -d autopilot-phase5-start 2>/dev/null`
