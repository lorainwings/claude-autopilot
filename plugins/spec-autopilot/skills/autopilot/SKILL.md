...
**并行模式**（当 `config.phases.testing.parallel.enabled = true`）：按测试类型（unit/api/e2e/ui）并行派发子 Agent，每个注入 Phase 1 需求追溯 + 对应 test_suites 配置。详见 `references/parallel-dispatch.md` Phase 4 模板。
...
读取 `references/phase5-implementation.md` 并行模式章节 + `references/parallel-dispatch.md` Phase 5 模板。
...
**并行测试执行**：按 `config.test_suites` 分套件并行派发（详见 `references/parallel-dispatch.md` Phase 6 模板）。
...
| 路径 | 内容 | 参考文档 |
|------|------|---------|
| A | Phase 6 测试执行 | `references/parallel-dispatch.md` Phase 6 模板 |
