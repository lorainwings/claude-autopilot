...
2.5. **检查并行执行配置** → 读取 `config.phases.implementation.parallel.enabled`
   - **启用** → 执行并行模式：
     a. 读取 `references/phase5-implementation.md` 并行执行模式章节
     b. 解析 `tasks.md`，构建 task 依赖图（基于文件引用和显式依赖）
     c. **文件所有权分区**：按顶级目录（backend/frontend/node）分组 task，跨目录 task 串行执行
     c1. **文件级锁注册**（v3.1.0）：将每个 agent 的 owned_files 注册到 file-locks.json，Hook 级别强制执行
     c2. 为每个并行 agent 生成 owned_files 列表，写入 `phase5-ownership/agent-{N}.json`
     d. 对每个 task 组：
        - 使用 `Task(isolation: "worktree", run_in_background: true)` 并行派发（最多 max_agents 个）
        - 等待所有 agent 完成
        - 按 task 编号顺序合并 worktree（`git merge --no-ff`）
        - 合并冲突 → AskUserQuestion 展示冲突文件，选择处理方式
        - 运行 quick_check 验证
        - 写入每个 task 的 checkpoint（`phase5-tasks/task-N.json`）
     e. 全部 group 完成后，运行 full_test
     f. **降级决策**：任何 group 合并失败超过 3 个文件 → 回退 worktree → 切换串行模式
   - **禁用**（默认） → 使用下方串行策略
