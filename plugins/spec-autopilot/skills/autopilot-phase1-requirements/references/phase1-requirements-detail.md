...
>
> 作为 ResearchAgent 的条件子任务执行（历史上曾为独立 web_search Agent，已合并）。
...
> **depth gating 的关系**：`config.phases.requirements.research.depth` 是联网调研的**主开关**——`depth=standard` 时 `websearch_quota=0`，本规则引擎不会触发任何 WebSearch；只有当 `depth=deep`（或被 maturity=ambiguous / complexity=large 自动升级）时，下述 `search_policy` 才作为**辅助决策层**参与 query 数量与跳过判定。`search_policy` 字段在 standard 路径下属于遗留兼容字段，不影响实际执行。
...
搜索次数上限由 `config.phases.requirements.research.websearch_quota` 控制（默认 3；`depth=standard` 时强制为 0，`depth=deep` 时上限 3）。
...
> 联网调研结果**不单独输出** `web-research-findings.md`，而是作为 ResearchAgent 主产物 `research-findings.md` 的一个子章节追加，以保持统一产物模型。
...
> 详细契约参见 autopilot skill 的 parallel-phase1 章节之 SynthesizerAgent 四要素契约。本节聚焦 detail 文档侧的执行细节与 verdict 消费规则。
...
- Read: openspec/changes/<name>/context/research-findings.md
  （联网调研结果合并写入该文件的 `## Web Research Findings` 章节，不单独维护 `web-research-findings.md`）
