...
- **路径 B: 串行模式**（当 `parallel.enabled = false` 时执行，或从路径 A 降级后以前台 Task 形式执行）：
  > **约束**: 从路径 A 降级时，路径 B 仍通过 `Task(run_in_background: false)` 派发子 Agent，
  > **主线程不直接编码实施任何 task**。
  - 主线程逐个派发**前台 Task**（同步阻塞），按域动态选择 Agent：
...
- **三路并行**：Phase 6 测试执行与 Phase 6.5 代码审查、质量扫描在同一消息中同时派发
