...
`references/dispatch-prompt-template.md` 的 "Four-Field Task Contract" section。
...
**执行前读取**: `references/dispatch-prompt-template.md`（完整的 Prompt 构造模板，含 for-each 循环和模型路由注入）
...
**执行前读取**: `references/dispatch-model-routing.md`（完整的 5 步路由流程 + Banner + 事件发射）
...
**执行前读取**: `references/dispatch-rules-injection.md`（完整的注入模板 + 执行流程）
...
**执行前读取**: `references/dispatch-phase-prompts.md`（完整的各阶段 prompt 构造逻辑）
...
**执行前读取**: `autopilot/references/parallel-dispatch.md`（通用协议）+ 当前 Phase 对应的并行配置（Phase 1 见 `../autopilot-phase1-requirements/references/parallel-phase1.md`；Phase 4 见 `../autopilot-phase4-testcase/references/parallel-phase4.md`；Phase 5 见 `autopilot/references/parallel-phase5.md`；Phase 6 见 `../autopilot-phase6-report/references/parallel-phase6.md`）
