...
| Phase | tier | model | 理由 |
|-------|------|-------|------|
| 1 | standard | sonnet | 需求分析调研子 Agent 用 sonnet 即可 |
...
1. **开始处理每个子任务时**:
   `Bash('bash ${CLAUDE_PLUGIN_ROOT}/runtime/scripts/emit-task-progress.sh "{task_id}" running {index} {total} {mode}')`
2. **每个子任务完成时**:
   `Bash('bash ${CLAUDE_PLUGIN_ROOT}/runtime/scripts/emit-task-progress.sh "{task_id}" passed {index} {total} {mode}')`
3. **子任务失败时**:
   `Bash('bash ${CLAUDE_PLUGIN_ROOT}/runtime/scripts/emit-task-progress.sh "{task_id}" failed {index} {total} {mode}')`
```
