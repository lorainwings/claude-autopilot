...
#    even when decision_points is present.)
BAD_SCAN_NO_TECH='{"status":"ok","summary":"Scanned repo and got output files.","decision_points":[],"key_files":["a.ts"],"output_files":["context/project-context.md"]}'
...
# 6a-ter. Updated for C14: decision_points now REQUIRED. A minimal valid scan
#   must include decision_points (empty array permitted); conflicts_detected
#   remains optional. existing_patterns remains optional.
MIN_SCAN='{"status":"ok","summary":"Minimal valid scan post-C14 with empty decision_points.","decision_points":[],"tech_constraints":["Node >= 18"],"key_files":["src/index.ts"],"output_files":["context/project-context.md","context/existing-patterns.md","context/tech-constraints.md"]}'
event=$(wrap_event "1-scan" "$MIN_SCAN")
exit_code=0
output=$(run_hook "$event") || exit_code=$?
assert_exit "minimal scan with empty decision_points → exit 0" 0 "$exit_code"
assert_not_contains "minimal scan with empty decision_points → no block" "$output" "$BLOCK_NEEDLE"
