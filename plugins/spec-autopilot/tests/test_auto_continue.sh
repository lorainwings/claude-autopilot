...
autopilot_content=$(cat "$AUTOPILOT_SKILL")
assert_contains "2d: autopilot 包含连续执行硬约束" "$autopilot_content" '连续执行硬约束'
assert_contains "2e: autopilot 禁止阶段完成后询问下一步" "$autopilot_content" '禁止.*是否继续下一阶段'
assert_contains "2f: autopilot 禁止审查输出元问题" "$autopilot_content" '是否先审查当前阶段输出'
assert_contains "2g: Step 1.5 显式 else 直达 Step 2" "$autopilot_content" '直接跳过 Step 1.5 进入 Step 2'
...
phase1_req_content=$(cat "$PHASE1_REQ")
assert_contains "4c: Phase 1 确认后立即进入 1.9" "$phase1_req_content" '选"确认".*立即.*进入 1.9'
assert_contains "4d: Phase 1 禁止下一步元问题" "$phase1_req_content" '下一步是 Phase 2'
assert_contains "4e: Phase 1 强制连续执行" "$phase1_req_content" '主线程必须继续进入下一执行阶段'

# --- 4f-4h. Phase 6.5 / Phase 5 内部也不应默认打断自动流 ---
PHASE6_REVIEW="$SKILL_DIR/autopilot/references/phase6-code-review.md"
phase6_review_content=$(cat "$PHASE6_REVIEW")
assert_contains "4f: code review warning 不再 AskUser" "$phase6_review_content" '不弹 AskUserQuestion'
assert_contains "4g: code review 统一由 Phase 7 readiness 判定" "$phase6_review_content" 'archive-readiness 统一判定'

PHASE5_IMPL="$SKILL_DIR/autopilot/references/phase5-implementation.md"
phase5_impl_content=$(cat "$PHASE5_IMPL")
assert_not_contains "4h: Phase 5 任务清单不再 AskUser 确认" "$phase5_impl_content" '通过 AskUserQuestion 展示任务清单让用户确认'
assert_contains "4i: Phase 5 默认直接进入执行" "$phase5_impl_content" '默认直接进入执行'
