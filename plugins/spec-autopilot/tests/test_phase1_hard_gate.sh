...
exit_code=0
output=$(
  unset AUTOPILOT_PHASE_ID
  echo '{"tool_name":"Task","tool_input":{"prompt":"Search for TODOs"},"tool_response":"Found 10 TODOs"}' |
    bash "$SCRIPT_DIR/post-task-validator.sh" 2>/dev/null
) || exit_code=$?
