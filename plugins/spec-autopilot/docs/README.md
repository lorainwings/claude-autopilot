...
> 本索引覆盖 spec-autopilot v5.1.0 全部文档，按用途分类。含事件总线、GUI V2 大盘、并行调度、需求路由等新特性。

## 入门指南 (`getting-started/`)

| 文档 | 说明 |
|------|------|
| [quick-start.md](getting-started/quick-start.md) | 5 分钟快速开始：安装到首次交付 |
| [integration-guide.md](getting-started/integration-guide.md) | 项目接入指南：全新项目完整接入流程 |
| [configuration.md](getting-started/configuration.md) | 配置参考：YAML 字段完整说明 |

## 架构参考 (`architecture/`)

| 文档 | 说明 |
|------|------|
| [overview.md](architecture/overview.md) | 架构总览：8 阶段流水线、三层门禁、事件总线、GUI V2、并行调度、需求路由 |
| [phases.md](architecture/phases.md) | 阶段详解：输入输出、checkpoint 格式、需求路由、TDD 循环、事件发射 |
| [gates.md](architecture/gates.md) | 门禁系统：三层门禁、反合理化 (16 种模式)、routing_overrides、decision_ack 闭环 |

## 运维指南 (`operations/`)

| 文档 | 说明 |
|------|------|
| [config-tuning-guide.md](operations/config-tuning-guide.md) | 配置调优：按项目类型优化配置 |
| [troubleshooting.md](operations/troubleshooting.md) | 故障排查：常见错误、调试技巧、恢复方案 |
