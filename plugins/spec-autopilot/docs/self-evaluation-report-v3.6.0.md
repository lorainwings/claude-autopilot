...
| autopilot-init | 配置初始化 | 合理，独立使用场景 |
...
**复杂度**：偏高。新用户面对 60+ 字段的配置文件时认知负担重。虽然 autopilot-init 的 Interactive Wizard（3 个预设模板）缓解了首次配置的门槛，但后续调优仍需理解大量字段间的交互关系（如 `test_pyramid.hook_floors` vs `test_pyramid.min_unit_pct` 的双层阈值）。
...
1. 安装插件
2. 运行 `/autopilot <需求>` → 自动触发 autopilot-init
